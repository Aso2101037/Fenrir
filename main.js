...
function renderJson(json) {
  ...
  main.innerHTML = "";
  span.innerHTML = "";
  h2.textContent = `${json.results.results_available}件見つかりました。距離の近い順に表示しています。`;
  ...
  for (let i = 0; i < json.results.shop.length; i++) {
    ...
    clone.querySelector(".name a").textContent = `店名：${json.results.shop[i].name}`;
    clone.querySelector(".name a").href = json.results.shop[i].urls.pc;
    clone.querySelector(".access a").textContent = `住所：${json.results.shop[i].address}`;
    clone.querySelector(".access a").href = json.results.shop[i].coupon_urls.pc;
    clone.querySelector(".average").textContent = `予算：${json.results.shop[i].budget.average}`;
...
function renderJson(json) {
  ...
  for (let i = 0; i < json.results.shop.length; i++) {
    ...
    clone.querySelector(
      ".name a"
    ).href = json.results.shop[i].urls.pc;
    clone.querySelector(
      ".address a"
    ).textContent = `住所：${json.results.shop[i].address}`;
    // clone.querySelector(
    //   ".access a"
    // ).href = json.results.shop[i].coupon_urls.pc;
    // clone.querySelector(
    //   ".average"
    // ).textContent = `予算：${json.results.shop[i].budget.average}`;
    clone.querySelector(
      ".access"
    ).textContent = `交通アクセス：${json.results.shop[i].access}`;
    clone.querySelector(
      ".photo img"
    ).src = `${json.results.shop[i].photo.pc.l}`;
    // clone.querySelector(
    //   ".open"
    // ).textContent = `営業時間：${json.results.shop[i].open}`;
    // clone.querySelector(
    //   ".large_area"
    // ).textContent = `エリア：${json.results.shop[i].large_area}`;
